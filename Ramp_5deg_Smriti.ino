...
void loop() {
  ...
  */
  ...
  int target = 5; 
    if (micros()>6000000 && micros()<50000000){
      digitalWrite(LED_BUILTIN, HIGH);
      posOffset = 0;

      k = 0.25; // works at 11V
      if (fabs(desiredAngle - target) < 4) k = 0.1;
      ...
      */
      ...
      desiredAngle = constrain(desiredAngle, 0, target);  // don't overshoot
    ...
    if (fabs(desiredAngle - target_down) > 0.05) {
        ...
        desiredAngle = constrain(desiredAngle, 0.0, 5.0);
    } else {
        desiredAngle = 0;

        encoderCount1 = -10;
        encoderCount2 = -10;
        integral = constrain(integral, -150, 0); // bleed off any forward-pushing integral
    }
