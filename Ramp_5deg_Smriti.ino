...
void loop() {
  ...
  */
  ...
  int target = 5; 
    if (micros()>6000000 && micros()<55000000){
    ...
    if (fabs(desiredAngle - target_down) > 0.05) {
        ...
        desiredAngle = constrain(desiredAngle, -1.0, 5.0);
    } else {
        desiredAngle = 0;

        encoderCount1 = 0;
        encoderCount2 = 0;
        integral = constrain(integral, -150, 150); // bleed off any integral
    }
