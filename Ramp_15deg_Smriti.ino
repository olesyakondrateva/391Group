...
void loop() {
  ...
  */
  ...
  int target_15 = 13;
    ...
    if (fabs(desiredAngle - target_down) > 0.15) {
        desiredAngle += (target_down - desiredAngle) * k_down * dt;
        desiredAngle = constrain(desiredAngle, -3.0, target_15);
    } else {
        desiredAngle = 0;

        encoderCount1 = 0;
        encoderCount2 = 0;
        integral = constrain(integral, -450, 450); // bleed off any forward-pushing integral
    }
    // posOffset active, but clamp it so it can't push angle positive
    //posOffset = constrain(posOffset, -0.5, 0.5);
    }
