  ...
  */
  ...
  int target_15 = 13.25;
    if (micros()>6000000 && micros()<40000000){
      ...
      k = 0.25; // works at 11V
      if (fabs(desiredAngle - target_5) < 4) k = 0.1;
    ...
    else if (micros()>40000000 && micros()<90000000){
      digitalWrite(LED_BUILTIN, HIGH);
      posOffset = 0;

      k = 0.20; // works at 11V
      if (fabs(desiredAngle - target_10) < 8.5) k = 0.075;
    ...
    else if (micros()>90000000 && micros()<150000000){
      digitalWrite(LED_BUILTIN, HIGH);
      posOffset = 0;

      k = 0.20; // works at 11V
      if (fabs(desiredAngle - target_15) < 12.5) k = 0.075;
      ...
      */
      ...
      desiredAngle = constrain(desiredAngle, 0, target_15);  // don't overshoot

    }
    }


    else if (micros()>150000000){
    ...
    if (fabs(desiredAngle - target_down) > 0.05) {
        ...
        desiredAngle = constrain(desiredAngle, 0.0, target_15);
    } else {
        desiredAngle = 0;

        encoderCount1 = 0;
        encoderCount2 = 0;
        integral = constrain(integral, -450, 0); // bleed off any forward-pushing integral
    }
    // posOffset active, but clamp it so it can't push angle positive
    posOffset = constrain(posOffset, -0.5, 0.0);
    }
