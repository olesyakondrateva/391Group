...
void loop() {
  ...
  //Serial.print(encoderCount1); Serial.print("     ");
  //Serial.println(encoderCount2);
  // -------- PID control --------

int pwm = (int)MotorController(desiredAngle - filterAngle, dt);
...
void driveMotors(int pwm) {
  ...
  if (pwmA > 5) pwmA += DEAD_A;
  else if (pwmA < -5) pwmA -= DEAD_A;

  if (pwmB > 5) pwmB += DEAD_B;
  else if (pwmB < -5) pwmB -= DEAD_B;
